...
#[test]
fn route_inventory_excludes_product_owned_billing_surfaces() {
    let routes = collect_routes();
    for route in HOSTED_BILLING_ROUTES {
        assert!(
            !routes.contains(*route),
            "Palette route inventory must not include product-owned billing route {route}",
        );
    }
}

#[tokio::test]
async fn router_does_not_register_product_owned_billing_surfaces() {
    ...
    for (method, path) in [
        ...
        assert_eq!(
            ...
            "Palette router must not register product-owned billing route {method} {path}",
        );
    }
}
