...
Status: stale pre-hardening maintainer diagnostic. The browser recording listed
below is 3.68s long and does not satisfy the current 8-second reviewability
floor. This file must be regenerated by the default-port compose stopwatch path
before it can support Gate 2 evidence.
