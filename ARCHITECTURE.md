...
## 27. Integrations & the Agent-Tooling Ecosystem
...
where we **deliberately don't integrate**, we say so (§27.4) rather than leaving a
silent blind spot. Tracked end-to-end in **#275** (this catalog's working issue),
with #276 (crate wiring) and #277 (MCP loop-closing) as the code companions.

### 27.0 Design stance — passive OTLP is necessary but not sufficient
...
### 27.1 Inbound — frameworks & instrumentation
...
### 27.2 Inbound — migration on-ramps ("switch from X")
...
### 27.3 Bidirectional — tool registries & agent protocols
...
### 27.4 What we deliberately don't integrate (yet)
...
(§27.3) is gated on #277's composite tools shipping first; we will not expose a
